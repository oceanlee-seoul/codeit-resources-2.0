...
export default function useGetAllSeatsData() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const [
        ...
        getSeatReservationListByDate(pickedDate),
  ...
  useEffect(() => {
    ...
    fixedResourceArray.forEach((resource) => {
      ...
      if (userData && owner === userData.id) {
        setMySeatInfo({ status: "fixed", seatName: name });
      }
    });
    ...
    reservationArray.forEach((reservation) => {
      const { id, resourceSubtype, resourceName, participants } = reservation;
      ...
      if (updatedAllSeatsData[resourceSubtype as string]) {
        ...
        if (seatIndex !== -1) {
          ...
          updatedAllSeatsData[resourceSubtype as string][
            seatIndex
          ].participant = oneAndOnlyParticipant;
        }
      }

      if (userData && participants?.includes(userData.id)) {
        setMySeatInfo({
          status: "confirmed",
          reservationId: id,
